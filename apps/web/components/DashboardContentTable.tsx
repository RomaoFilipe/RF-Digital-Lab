    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.03]">
      <div className="overflow-x-auto">
        <div className="min-w-[760px]">
          <div className="grid grid-cols-[minmax(0,1fr)_120px_120px_220px] gap-4 border-b border-white/10 px-6 py-3 font-mono text-[10px] uppercase tracking-[0.28em] text-slate">
            <span>Título</span>
            <span>Status</span>
            <span>Tipo</span>
            <span>Ações</span>
          ...
          {items.map((item) => (
            <div
              key={item.id}
              className="grid grid-cols-[minmax(0,1fr)_120px_120px_220px] gap-4 border-b border-white/10 px-6 py-4 text-sm text-sand-2 last:border-b-0"
            >
              <span className="flex min-w-0 items-center gap-2 text-wrap-anywhere">
                {item.title}
